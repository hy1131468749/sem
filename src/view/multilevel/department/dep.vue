    ...
    data () {
      ...
      methods: {
        ...
        edit () {
          ...
          this.$router.push( { name: 'edit_dep',
            params: {
              id: check[0].id
            }
          });
        ...
        deleteById (){
          ...
          axios.get('department/delete',{params: {ids: ids}})
        .then(respon => {
        ...
        axios.get('department/findList',{params:params})
          .then(response =>{
    ...
    mounted() {
      ...
      const setting = {
        ...
        async: {
          enable: true,
          type: 'get',
          url: `${global_config.httpUrl}/department/findZtree`,
          autoParam: ['id']
        }

      };
      bootstrapUtils.initTable('a1', tableConfig);
      axios.get(`department/findZtree?id=${this.parentId}`).
      then(reponse => {
